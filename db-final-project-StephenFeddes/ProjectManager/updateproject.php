<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">

    <title>Update Project</title>
  </head>
  
  <body>
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark" aria-label="Fifth navbar example">
        <div class="container-fluid">
            <a class="navbar-brand" href="#">SoftCo</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbar"
                aria-controls="navbarsExample05" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>

            <div class="collapse navbar-collapse" id="navbar">
                <div class="navbar-nav me-auto mb-2 mb-lg-0">
                    <a class="nav-link" aria-current="players" href="createproject.php">Create Project</a>
                    <a class="nav-link" href="updateproject.php">Update Project</a>
                    <a class="nav-link" href="dropproject.php">Drop Project</a>
                    <a class="nav-link" href="projectoverview.php">Project OverView</a>
                </div>
                <div class="navbar-nav ms-auto">
                    <a class="nav-link" aria-current="page" href="departments.php">Departments</a>
                    <a class="nav-link" aria-current="page" href="employees.php">Employees</a>
                    <a class="nav-link" aria-current="page" href="createemployee.php">Create Employee</a>
                    <a class="nav-link" aria-current="page" href="dropemployee.php">Drop Employee</a>
                    <a class="nav-link" aria-current="page" href="assignemployee.php">Assign Employee</a>
                </div>
            </div>
        </div>
    </nav>

<div class="row">
      <div class="col-md-6 mx-auto">
        <div class="card card-body bg-light mt-5">
          <h2>Update a Project</h2>
          <p>Name a project and update its budget and/or due date</p>
        
          <form action="updateproject.php" method="post">

            <div class="form-group">
                <label for="ProjectName">Project Name:</label>
                <input type="text" name="ProjectName" class="form-control form-control-lg">
            </div>
            <div class="form-group">
                <label for="Budget">Budget:</label>
                <input type="number" name="Budget" class="form-control form-control-lg">
            </div>
            <div class="form-group">
              <label for="DueDate">Due Date:</label>
              <input type="date" id="DueDate" name="DueDate">
            </div>

            <div class="row">
              <div class="col">
                <input type="submit" value="Update" class="btn btn-primary btn-block">
              </div>
            </div>
          </form>
        </div>
      </div>
    </div>

    <?php
    // Does not let users see an error on the webpage
    ini_set('display_errors', 0);
    
    // Set up PDO driver to access the database created in projectmanager.sql
    $dsn = "mysql:host=localhost;dbname=project";
    $username = "planner";
    $password = "coolproject";
    $pdo = new PDO($dsn, $username, $password);
    
    # Gets the user input from the forms and stores it an array
    $_POST = filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);

    # Trims the user input and adds it to a new array
    $data = [
      "ProjectName" => trim($_POST["ProjectName"]),
      "Budget" => trim($_POST["Budget"]),
      "DueDate" => trim($_POST["DueDate"]),
    ];


    // If the user updates the project's budget and due date, both will be updated
    if ($data["Budget"] != "" and $data["DueDate"] != "") {
      $data["DueDate"] = date("Y-m-d", strtotime($data["DueDate"]));
      $sql1 = "UPDATE Project " .
      "SET Budget = :budget, DueDate = :dueDate " .
      "WHERE ProjectName = :projectName";
      $statement = $pdo->prepare($sql1);
      $statement->bindValue("budget", $data["Budget"]);
      $statement->bindValue("dueDate", $data["DueDate"]);
      $statement->bindValue("projectName", $data["ProjectName"]);
      $statement->execute();
      echo "<p style='text-align:center'>Budget and due date successfully updated</p>";
    }

    // If the user only updates the budget, only the budget will be updated
    else if ($data["Budget"] != "") {
        $sql2 = "UPDATE Project " .
       "SET Budget = :budget " .
       "WHERE ProjectName = :projectName";
       $statement = $pdo->prepare($sql2);
       $statement->bindValue("budget", $data["Budget"]);
       $statement->bindValue("projectName", $data["ProjectName"]);
       $statement->execute();
       echo "<p style='text-align:center'>Budget successfully updated</p>";
    }
    // If the user only updates the due date, only the due date will be updated
    else if ($data["DueDate"] != "") {
        $data["DueDate"] = date("Y-m-d", strtotime($data["DueDate"]));
        $sql3 = "UPDATE Project " .
       "SET DueDate = :dueDate " .
       "WHERE ProjectName = :projectName";
       $statement = $pdo->prepare($sql3);
       $statement->bindValue("dueDate", $data["DueDate"]);
       $statement->bindValue("projectName", $data["ProjectName"]);
       $statement->execute();
       echo "<p style='text-align:center'>Due date successfully updated</p>";
    }
    else {
        echo "<p style='text-align:center'>No update has happened</p>";
    }
  ?>

<div class="px-4 py-5 my-5 text-center">
    <h1 class="fw-bold" style="margin: 0px">Projects:</h1>

    <div style="margin: 20px">
        <?php
          $sql4 = "SELECT ProjectName FROM Project";
          $statement = $pdo->query($sql4);

        // Shows the current available projects
        while ($row = $statement->fetch()) {
            echo "<div class='container-fluid'><b>$row[ProjectName]</b></div>";
        }
        ?>
    </div>
</div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
</body>
</html>